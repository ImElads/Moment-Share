...
public class Model {
    ...
    public interface IModelUpdate {
        ...
        void createEventCompleted(Exception e);
    }
    ...
    private void raiseMediaPostChanged(Exception e, Event event){
        for (IModelUpdate modelUpdate : modelUpdates){
            modelUpdate.mediaPostChanged(e, event);
        }
    }
    ...
    private void raiseCreateEventCompleted(Exception e){
        for (IModelUpdate modelUpdate : modelUpdates){
            modelUpdate.createEventCompleted(e);
        }
    }
        ...
        eventsRef.document(event.getId()).collection("Posts").orderBy("uploadTime").get()
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    ...
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        ...
                        raiseMediaPostChanged(null ,event);
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        raiseMediaPostChanged(e, event);
                    }
                });
    }

    //used by load data to ensure both user data and event data are retrieved before calling observer
    private void checkIfAllRequestsCompleted(AtomicInteger pendingRequests) {
        // If pending requests reach zero, call raiseEventDataChanged
        int num = pendingRequests.decrementAndGet();
        if (num <= 0) {
            raiseEventDataLoaded(null);  // Notify that all events are loaded
        }
    }


    private void loadEventsFromFB(){
        final AtomicInteger pendingRequests = new AtomicInteger(2);
        ...
        eventsRef.whereArrayContains("participantIds", currentUser.getId()).get()
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    ...
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        ...
                        checkIfAllRequestsCompleted(pendingRequests);
                    }
        ...
        //find events where the user is the host
        eventsRef.whereEqualTo("hostId", currentUser.getId()).get()
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    @Override
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        //load events
    ...
    public void joinEvent(String eventCode) {

        // Reference the specific event document by its ID (eventCode)
        DocumentReference eventRef = db.collection("events").document(eventCode);
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && task.getResult().exists()) {
                        ...
                        if (participantIds.contains(currentUser.getId())) {
                            Log.e("JoinEvent", "User is already a member of this event");
                            return;
                        }
    ...
    // adds event to user outside the events array
    public void createEvent(Event event, Bitmap bitmap) {
        ...
        storageRef.putFile(fileUri)
                .addOnSuccessListener(taskSnapshot -> {
                    ...
                    storageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                        ...
                        newEventRef.set(event)
                                .addOnSuccessListener(aVoid -> Log.d("Firestore", "Event with image saved successfully"))
                                .addOnFailureListener(e -> Log.e("Firestore", "Error saving event with image", e));
                    });
                })
                .addOnFailureListener(e -> Log.e("CreateEvent", "Failed to upload event image", e));
    ...
    public void addPostToEvent(String eventId, MediaPost mediaPost) {
        ...
        postsRef.add(mediaPost)
                .addOnSuccessListener(documentReference -> {
                    ...
                    raiseMediaPostChanged(null, getEvent(eventId));
    ...
    public void deleteMediaPost(String eventId, String postId) {
        ...
        postRef.delete()
                ...
                .addOnFailureListener(e -> {
                    ...
                    raiseMediaPostChanged(e, getEvent(eventId));
                });
    }
