...
public class EventSharedAlbumFragment extends Fragment implements Model.IModelUpdate {
    ...
    public void deleteMediaPost(MediaPost mediaPost){
        for (MediaPost mp : event.getMediaPosts()){
            if (mp.getId().equals(mediaPost.getId()))
                event.getMediaPosts().remove(mediaPost);
        }
        mediaPostsAdapter.notifyDataSetChanged();
    }
